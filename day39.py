...
#如果某原因只集中在很小区域，说明它是局部热点问题
#如果某原因分布很广、频次很高，说明它是整体主导瓶颈
...
# =========================================================
# 11. 扫描 dose 和 threshold，并收集统计信息
# =========================================================
...
for i, threshold in enumerate(threshold_list):
    for j, dose in enumerate(dose_list):
        ...
        first_reason = reason_list[0]
        reason_map[i, j] = reason_to_code[first_reason]


        # 统计整张图的原因列表
        all_reason_lists.append(reason_list)
...
# 12. 做频次统计
# =========================================================
all_reason_count = count_reason_frequency(all_reason_lists)
fail_reason_count = count_reason_frequency(fail_reason_lists)
borderline_reason_count = count_reason_frequency(borderline_reason_lists)
...
border_labels = list(borderline_reason_count.keys())
border_values = list(borderline_reason_count.values())
all_labels = list(all_reason_count.keys())
all_values = list(all_reason_count.values())
...
if len(fail_labels) > 0:
    ...
    plt.xticks(range(len(fail_labels)), fail_labels, rotation=45, ha='right')
...
summary_text = (
    ...
    "[Whole-map status count]\n"
    f"fail points = {num_fail}\n"
    f"borderline points = {num_borderline}\n"
    f"pass points = {num_pass}\n\n"
    "[Top fail reasons]\n"
    f"{fail_reason_count}\n\n")
...
if len(border_labels) > 0:
    ...
    plt.xticks(range(len(border_labels)), border_labels, rotation=45, ha='right')
...
if len(all_labels) > 0:
    plt.bar(range(len(all_labels)), all_values)
    plt.xticks(range(len(all_labels)), all_labels, rotation=45, ha='right')

plt.title("all Reason Frequency")
plt.xlabel("reason")
plt.ylabel("count")
plt.tight_layout()
plt.show()