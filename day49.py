#从单一 worst EPE 到综合目标函数 objective
import numpy as np
...
# 7. 今天的新函数：综合目标函数
# =========================================================
def compute_objective(mean_abs_epe, worst_abs_epe, worst_weight=2.0):
    """
    综合目标函数：
        objective = mean |EPE| + worst_weight * worst |EPE|
    ...
    worst_weight:
        表示我们有多重视 worst-case / hotspot
    """
    objective = mean_abs_epe + worst_weight * worst_abs_epe
    return objective


# =========================================================
# 8. 带 objective 的稳定迭代 OPC 参数
# =========================================================
...
objective_current = compute_objective(
    mean_abs_current,
    worst_abs_current,
...
# =========================================================
# 10. 用 objective 做 accept / reject 的主循环
# =========================================================
for it in range(max_iters):
    ...
    objective_history.append(objective_current)
    ...
    objective_trial = compute_objective(
        mean_abs_trial,
        worst_abs_trial,
        worst_weight=worst_weight
    )
    ...
    # =====================================================
    # 关键变化：
    # 第48天看 worst_abs_trial
    # 第49天看 objective_trial
    # =====================================================
    if objective_trial < objective_current:
        # 接受 trial
        ...
        objective_current = objective_trial
...
objective_initial = compute_objective(
    mean_abs_initial,
    worst_abs_initial,
    worst_weight=worst_weight
)
...
plt.title("Printed Final with Objective-based OPC")
plt.xlabel("x")
plt.ylabel("y")
...
plt.plot(range(len(objective_history)), objective_history, marker='s', label='objective')
...
summary_text = (
    "[Objective-based Iterative Toy OPC]\n"
    f"worst_weight = {worst_weight:.2f}\n"
    f"initial gain = 0.20\n"
    ...
    f"objective initial = {objective_initial:.4f}\n"
    f"objective final   = {objective_final:.4f}\n\n"
    "[Objective]\n"
    "objective = mean |EPE| + weight * worst |EPE|"
)
