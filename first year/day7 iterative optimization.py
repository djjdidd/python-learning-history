...
def serif_objective_soft(size_array):
...
def numerical_gradient(theta, h=0.01):

    loss_plus = serif_objective_soft(
        np.array([theta + h])
    )

    loss_minus = serif_objective_soft(
        np.array([theta - h])
    )
