#多指标整体判定规则：把比例型指标和 worst-case 指标放在同一个整体规则里，才能同时考虑整体趋势和局部极端退化
import numpy as np
...
def analyze_pattern_rows(
    ...
    num_features_rows = np.full(ny, np.nan)
    ...
    for i in range(ny):
        ...
        num_features_rows[i] = num_features
    ...
    return row_status, left_cd_rows, gap_cd_rows, right_cd_rows, num_features_rows, summary
...
def evaluate_global_status_multi_metric(
    ...
    if (not np.isnan(min_left_cd)) and (min_left_cd < fail_left_limit):
        return 0#先确认 min_gap_cd 不是 nan。因为如果它是 nan，说明没有有效值，不能直接拿去比较
...
for i, threshold in enumerate(threshold_list):
    for j, dose in enumerate(dose_list):
        ...
        row_status, left_cd_rows, gap_cd_rows, right_cd_rows, num_features_rows, summary = analyze_pattern_rows(
            printed,
            x,
            y,
            y_min_pattern,
...
row_status_ex, left_cd_rows_ex, gap_cd_rows_ex, right_cd_rows_ex, num_features_rows_ex, summary_ex = analyze_pattern_rows(
    printed_example,
    x,
    y,
    y_min_pattern,
...
status_ex = evaluate_global_status_multi_metric(
    fail_ratio=summary_ex["fail_ratio"],
    weak_ratio=summary_ex["weak_ratio"],
    min_left_cd=summary_ex["min_left_cd"],
    min_gap_cd=summary_ex["min_gap_cd"],
    min_right_cd=summary_ex["min_right_cd"],
    fail_tol=fail_tol,
    weak_tol=weak_tol,
    fail_left_limit=fail_left_limit,
    weak_left_limit=weak_left_limit,
...
plt.axhline(fail_gap_limit, linestyle='--')
plt.axhline(weak_gap_limit, linestyle='--')
...
summary_text = (
    ...
    f"global status = {status_name[status_ex]}"
)
